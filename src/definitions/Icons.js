import React from "react";
import {Icon} from "@ui-kitten/components";

export const SearchIcon = props => <Icon {...props} name={"search"}/>;
export const StarIcon = props => <Icon {...props} name={"star"}/>;
export const MarsIcon = props => <Icon {...props} name={"globe-2"}/>;
export const NewsIcon = props => <Icon {...props} name={"cast"}/>;

export const SunIcon = props => <Icon {...props} name={"sun"}/>;
export const MoonIcon = props => <Icon {...props} name={"moon"}/>;
export const BackIcon = props => <Icon {...props} name={"arrow-ios-back"}/>;

export const HeartOutlineIcon = props =>  <Icon {...props} name={"heart-outline"}/>;
export const HeartFillIcon = props =>  <Icon {...props} name={"heart"}/>;